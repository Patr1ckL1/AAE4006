...
%    bz_hat      = uu(49);            % estimated z-gyro bias
    ...
    if t==0
        ...
        subplot(8,2,1)
        hold on
        pn_handle = graph_y_yhat_yd(t, pn, pn_c, 'p_n', []);
        legend('Real','Reference')
        
        subplot(8,2,2)
        hold on
        Va_handle = graph_y_yhat_yd(t, Va, Va_c, 'V_a', []);

        subplot(8,2,3)
        hold on
        pe_handle = graph_y_yhat_yd(t, pe,  pe_c, 'p_e', []);

        subplot(8,2,4)
        hold on
        alpha_handle = graph_y_yhat_yd(t, alpha, alpha_c, '\alpha', []);

        subplot(8,2,5)
        hold on
        h_handle = graph_y_yhat_yd(t, h, h_c, 'h', []);

        subplot(8,2,6)
        hold on
        beta_handle = graph_y_yhat_yd(t, beta, beta_c, '\beta', []);

        subplot(8,2,7)
        hold on
        phi_handle = graph_y_yhat_yd(t, phi, phi_c, '\phi', []);
        
        subplot(8,2,8)
        hold on
        p_handle = graph_y_yhat_yd(t, p, p_c, 'p', []);
        
        subplot(8,2,9)
        hold on
        theta_handle = graph_y_yhat_yd(t, theta, theta_c, '\theta', []);
        
        subplot(8,2,10)
        hold on
        q_handle = graph_y_yhat_yd(t, q, q_c, 'q', []);
        
        subplot(8,2,11)
        hold on
        chi_handle = graph_y_yhat_yd(t, chi, chi_c, '\chi', []);
        
        subplot(8,2,12)
        hold on
        r_handle = graph_y_yhat_yd(t, r, r_c, 'r', []);
    ...
    % at every other time step, redraw state variables
       ...
       graph_y_yhat_yd(t, pn, pn_c, 'p_n', pn_handle);
       graph_y_yhat_yd(t, pe, pe_c, 'p_e', pe_handle);
       graph_y_yhat_yd(t, h, h_c, 'h', h_handle);
       graph_y_yhat_yd(t, Va, Va_c, 'V_a', Va_handle);
       graph_y_yhat_yd(t, alpha, alpha_c, '\alpha', alpha_handle);
       graph_y_yhat_yd(t, beta, beta_c, '\beta', beta_handle);
       graph_y_yhat_yd(t, phi, phi_c, '\phi', phi_handle);
       graph_y_yhat_yd(t, theta, theta_c, '\theta', theta_handle);
       graph_y_yhat_yd(t, chi, chi_c, '\chi', chi_handle);
       graph_y_yhat_yd(t, p, p_c, 'p', p_handle);
       graph_y_yhat_yd(t, q, q_c, 'q', q_handle);
       graph_y_yhat_yd(t, r, r_c, 'r', r_handle);
...
function handle = graph_y_yhat_yd(t, y, yd, lab, handle)
